/*
    VitaSuwayomi — PS4 updater helper

    A tiny standalone PS4 app whose ONLY job is to install a VitaSuwayomi update
    while VitaSuwayomi itself is closed. This is the PS4 analogue of the Vita
    ...
    process. So VitaSuwayomi launches THIS helper and quits; with VitaSuwayomi no
    longer running, the helper uninstalls VSWY00002 (its data under
    /data/VitaSuwayomi is on a different partition and is untouched) and hands the
    ...
    in the notifications / Downloads list, and the user reopens VitaSuwayomi once it
    ...
    Everything is fixed by convention so no arguments cross the app boundary:
      - the pkg to install : /data/VitaSuwayomi/update.pkg
      - the app to replace : VSWY00002 (VitaSuwayomi's title id)
...
// Relaunching VitaSuwayomi when the install finishes. Layout per idc's
...
const char* kPkgPath  = "/data/VitaSuwayomi/update.pkg";
...
const char* kPkgPathSystem = "/user/data/VitaSuwayomi/update.pkg";
const char* kLogPath  = "/data/VitaSuwayomi/updater_ps4.log";
const char* kTargetId = "VSWY00002";

constexpr size_t kBgftHeapSize  = 1 * 1024 * 1024;
// SCE_BGFT_TASK_OPT_FORCE_UPDATE, per flatz's bgft.h. VitaSuwayomi is closed while
...
void uiInit() {
    ...
    // borealis demonstrably renders on this hardware (VitaSuwayomi logs
    ...
    g_win = SDL_CreateWindow("VitaSuwayomi Updater", SDL_WINDOWPOS_UNDEFINED,
...
void uiFrame(float t, float fraction = -1.0f) {
    ...
    fillRect(panelX, panelY, panelW, 8, 0x64, 0xB4, 0xFF, 0xFF);
    ...
    // is built from rows of rects, which is guaranteed to show up.
    {
        ...
        for (int i = 0; i < rows; ++i) {
            ...
            if (w > 0) fillRect(markX, markY + i * 2, w, 2, 0x64, 0xB4, 0xFF, 0xFF);
        }
    }
    ...
    if (fraction >= 0.0f) {
        ...
        if (w < 2) w = 2;
        if (w > barW) w = barW;
        fillRect(barX, barY, w, barH, 0x64, 0xB4, 0xFF, 0xFF);
    } else {
        ...
        fillRect(segX, barY, segW, barH, 0x64, 0xB4, 0xFF, 0xFF);
    }
...
int main(int, char*[]) {
    ...
    // nothing. VitaSuwayomi makes these same calls successfully because borealis
    // has already run this.
    int sdl = SDL_Init(SDL_INIT_VIDEO);
    vlog("ps4 updater: SDL_Init -> %d (%s)", sdl, sdl == 0 ? "ok" : SDL_GetError());

    // Give VitaSuwayomi a moment to fully exit before we uninstall its title.
    sleep(3);
    ...
    // launched by a quitting VitaSuwayomi never becomes the foreground/composited
    ...
    // the same way for a sandboxed app, and VitaSuwayomi only gets away with direct
    ...
    // Optional extras: used to wait for the install and relaunch VitaSuwayomi. If
    ...
    paramsEx.param.contentName      = "VitaSuwayomi";
    ...
    // console with no VitaSuwayomi at all — which is exactly what happened when the
    ...
    // VitaSuwayomi, after proving on-device that both fight the platform:
    ...
    // The user reopens VitaSuwayomi from the home screen once the install
    // notification finishes — the same flow every other PS4 homebrew uses.
    // VitaSuwayomi removes this helper on its next start (ps4::removeUpdaterApp).
    sleep(2);
    vlog("ps4 updater: done (install continues in the background; reopen "
         "VitaSuwayomi when the system notification finishes)");
    return 0;
}
