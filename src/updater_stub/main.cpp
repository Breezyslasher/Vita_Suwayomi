/*
    VitaSuwayomi — updater stub

    A tiny standalone Vita app whose ONLY job is to install a VitaSuwayomi
    update while VitaSuwayomi itself is closed, then relaunch it. This is the
    AutoPlugin2 technique: a title cannot promote over itself (the Vita
    installer returns 0x80101114 "in use" for the running app), so VitaSuwayomi
    installs and launches THIS stub, which — now that VitaSuwayomi is no longer
    the running title — can promote VitaSuwayomi's downloaded VPK cleanly, then
    boots VitaSuwayomi.

    Everything is fixed by convention so no arguments need to cross the app
    boundary:
      - the VPK to install : ux0:data/VitaSuwayomi/update.vpk
      - the target version : ux0:data/VitaSuwayomi/update_version.txt (optional)
      - the app to relaunch : VSWY00001 (VitaSuwayomi's title id)

    The heavy lifting (extract + head.bin forge + promote) is shared verbatim
    with VitaSuwayomi via utils/vita_install, which is deliberately free of any
    borealis / fmt dependency so this stub links it without the UI stack. The
    screen is a plain-vita2d step checklist.

    Ported from the VitaPlex updater stub (same AutoPlugin2 lineage).
*/
...
const char* kUpdateVpk  = "ux0:data/VitaSuwayomi/update.vpk";
const char* kVersionTxt = "ux0:data/VitaSuwayomi/update_version.txt";
const char* kWorkDir    = "ux0:data/VitaSuwayomi/updater_work";
...
#define C_ACCENT    RGBA8(0x64, 0xB4, 0xFF, 0xFF)   // #64B4FF
#define C_ACCENTBR  RGBA8(0x9A, 0xD2, 0xFF, 0xFF)
#define C_TILEBG    RGBA8(0x64, 0xB4, 0xFF, 26)
#define C_TILEBRD   RGBA8(0x64, 0xB4, 0xFF, 77)
#define C_ACCENTFILL RGBA8(0x64, 0xB4, 0xFF, 26)
#define C_GREEN     RGBA8(0x4E, 0xCC, 0xA3, 0xFF)   // #4ECCA3
#define C_GREENRING RGBA8(0x4E, 0xCC, 0xA3, 128)
#define C_GREENFILL RGBA8(0x4E, 0xCC, 0xA3, 31)
#define C_RED       RGBA8(0xD9, 0x6B, 0x6B, 0xFF)   // #d96b6b
#define C_REDFILL   RGBA8(0xD9, 0x6B, 0x6B, 26)
#define C_WHITE     RGBA8(0xFF, 0xFF, 0xFF, 0xFF)
#define C_BODY      RGBA8(0xC5, 0xC6, 0xD0, 0xFF)
#define C_MUTED     RGBA8(0x8B, 0x8B, 0x93, 0xFF)
...
struct Ui {
    ...
    std::string version;         // "Beta 2.2.1" or empty
    std::string sizeMB;          // "12.3" or empty
...
void render() {
    ...
    // Icon tile + border + download arrow.
    ...
    // Title: "Vita" white + "Suwayomi" accent + " Updater" white.
    {
        float x = 46, y = 262, s = 1.2f;
        vita2d_pgf_draw_text(g_font, x, y, C_WHITE, s, "Vita");
        x += vita2d_pgf_text_width(g_font, s, "Vita");
        vita2d_pgf_draw_text(g_font, x, y, C_ACCENT, s, "Suwayomi");
        x += vita2d_pgf_text_width(g_font, s, "Suwayomi");
    ...
    if (!g_ui.version.empty()) {
        ...
        drawWrapped(46, 316, 308, 0.85f, C_MUTED,
                    "VitaSuwayomi will restart automatically when this finishes.", 22);
    } else {
        drawWrapped(46, 294, 308, 0.85f, C_MUTED,
                    "Installing the update. VitaSuwayomi will restart automatically "
    ...
    Row rows[4] = {
        ...
        { "Relaunch VitaSuwayomi", "" },
    };

    const float colX = 469.0f;
...
int main(int, char*[]) {
    ...
    // Give the shell a moment to finish tearing down VitaSuwayomi so the
    // promoter no longer sees the title as in use.
    ...
    if (rc == 0) {
        ...
        g_ui.note = "Update kept at ux0:data/VitaSuwayomi/update.vpk - install it with VitaShell";
        render();
        sceKernelDelayThread(4 * 1000 * 1000);
    }

    // Relaunch VitaSuwayomi either way — the updated build on success, the
