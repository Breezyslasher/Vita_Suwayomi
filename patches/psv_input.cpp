...
void PsvInputManager::updateControllerState(ControllerState* state, int controller)
{
    ...
    // Fix: Use SCE_CTRL_L/R (shoulder buttons) instead of SCE_CTRL_L2/R2 (triggers)
    // The Vita only has L/R shoulder buttons, not L2/R2 triggers
    state->buttons[BUTTON_LB] = pad.buttons & SCE_CTRL_L;
    state->buttons[BUTTON_RB] = pad.buttons & SCE_CTRL_R;
